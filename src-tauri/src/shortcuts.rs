...
/// Shows the main overlay (used by shortcut handlers) without stealing OS focus
/// on Windows, while preserving the existing focus behaviour on macOS/Linux.
...
fn handle_toggle_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let visibility_result = if new_hidden {
            window
                .hide()
...
pub fn set_app_icon_visibility<R: Runtime>(app: AppHandle<R>, visible: bool) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(window) = app.get_webview_window("main") {
            ...
            // On Windows, changing the window style via set_skip_taskbar only
            // takes effect for the taskbar after the window is hidden and
            // re-shown. Without this, an already-visible window keeps its
            // existing taskbar button, so the toggle appears non-functional.
            // Only do this when the window is actually visible, otherwise a
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Some(window) = app.get_webview_window("main") {
            ...
            // Same workaround as Windows: the taskbar/panel entry for an
            // already-visible window only updates after a hide/show cycle.
            if window.is_visible().unwrap_or(false) {
                let _ = window.hide();
                let _ = window.show();
            }
